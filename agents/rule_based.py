...
"""
基于博弈树搜索的规则引擎智能体 (ADAgent)

核心特性:
  - Alpha-Beta 剪枝博弈树搜索 + 置换表 (Zobrist 哈希)
  - 模式匹配评估函数 (五连/活四/冲四/活三/眠三/活二/眠二)
  - VCT (Victory by Continuous Threats) 连续冲四搜索
  - 静态搜索 (Quiescence Search) 过滤不稳定局面
  - 增量更新: 走子/悔子时增量计算候选着法和全局评分
  - 26种标准五子棋开局定式
  - 防卡死机制: 节点数上限+超时保护

使用示例:
  from agents.rule_based import ADAgent
  agent = ADAgent(depth=4, max_candidates=10, vct_depth=8)
  move = agent.get_move(state)  # state: GameState

搬自: agent_ad.py (融合最终版)
"""
...
class PrivateState:
    ...
    包含棋盘的副本、评估分数、候选着法等增量更新的元数据。
    与 GameState (纯游戏数据) 分离，避免污染共享状态。
    """
    plate: bytearray                                   # 棋盘副本 (15x15, 扁平化)
    player: int                                        # 当前玩家 (1 黑 / 2 白)
    score_black: int = 0                               # 黑方全局评估分
    score_white: int = 0                               # 白方全局评估分
    candidates: Set[Tuple[int, int]] = field(default_factory=set)  # 候选着法集合
    history: List[Tuple[int, int]] = field(default_factory=list)    # 落子历史
    zhash: int = 0                                     # Zobrist 哈希值 (用于置换表索引)
...
_ZOBRIST_TABLE = [
    [[_ZOBRIST_RNG.getrandbits(64) for _ in range(3)] for _ in range(15)]
    for _ in range(15)
]

# ═══════════════════════════════════════════════════════════════
#  ADAgent: 基于博弈树的规则引擎智能体
# ═══════════════════════════════════════════════════════════════

class ADAgent(Agent):
    """
    基于 Alpha-Beta 博弈树 + 模式匹配评估的规则引擎智能体。
    
    实现 Agent 抽象基类接口，可直接接入竞技场、数据收集器等上层模块。
    
    典型配置:
      - depth=4: 适合数据收集（快速生成大量对局）
      - max_candidates=10: 每步搜索候选数
      - vct_depth=8: VCT 算杀深度（越深越慢但越强）
    """
    
    # 共享 Zobrist 表（类变量，多进程间各自导入）
    ZOBRIST_TABLE = _ZOBRIST_TABLE

    # ═══════════ 评估分数常量 ═══════════
    SCORE_FIVE = 10000000       # 五连
    SCORE_LIVE_FOUR = 100000    # 活四
    SCORE_RUSH_FOUR = 10000     # 冲四
    SCORE_LIVE_THREE = 5000     # 活三
    SCORE_SLEEP_THREE = 500     # 眠三
    SCORE_LIVE_TWO = 200        # 活二
    SCORE_SLEEP_TWO = 20        # 眠二
    SCORE_LIVE_ONE = 10         # 活一
    SCORE_SLEEP_ONE = 1         # 眠一

    # ═══════════ 模式匹配表 ═══════════
    # 格式: (模式串, 分数)
    #   '1' = 己方棋子, '0' = 空位, '2' = 对方棋子或边界
    _PATTERNS = [
        ('11111',  SCORE_FIVE),
    ...
    # ═══════════ 26种标准五子棋开局定式 ═══════════
    # 格式: 白2位置 -> [定式变例列表]
    #   每个变例: {"name": 定式名, "b3": 黑3位置, "w4": [白4候选位置]}
    OPENING_BOOK = {
    ...
    def __init__(self, depth: int = 4, max_candidates: int = 10, use_quiescence: bool = True,
        ...
        Args:
            depth: Alpha-Beta 搜索深度（降低→提速，升高→提质量）
            max_candidates: 每步候选着法数
            use_quiescence: 是否启用静态搜索（过滤不稳定局面）
            vct_depth: VCT 连续冲四搜索深度
            quiescence_depth: 静态搜索深度
            max_trans_size: 置换表最大条目数
            max_nodes: 搜索节点上限（防卡死）
            name: 智能体名称（用于竞技场显示）
        """
        self.name = name
        ...
        # 置换表 (LRU 淘汰策略): key=Zobrist哈希, value=(depth, flag, score, best_move)
        self.trans_table = OrderedDict()
        self.node_count = 0  # 本局已搜索节点数
        self.timeout = False  # 是否超时/超节点
    ...
    def _build_private_state(self, state: GameState) -> PrivateState:
        """从 GameState 完整构建 PrivateState（用于首次或增量失效时）。"""
    ...
    def _generate_candidates(self, plate, history):
        """生成候选着法：距离已有棋子 Manhattan 距离 ≤2 的空位。"""
        if not history:
            return {(7, 7)}
        cand = set()
        for r in range(15):
    ...
    def _evaluate_line(self, line):
        """
        对一行（5个以上连续位置）进行模式匹配评估。
        
        分别以己方和对方视角编码，匹配预定义模式表。
        使用 used 数组防止重叠匹配（优先级由 _PATTERNS 顺序决定）。
        """
        score_b, score_w = 0, 0
        n = len(line)
        if n < 5:
    ...
    def _sort_candidates(self, state, trans_best=None):
        """按紧迫度排序候选着法：五连 > 防五连 > 活四防守 > 活四进攻 > 按评分排序。"""
        ...
        for act in state.candidates:
            ...
            if my >= self.SCORE_FIVE:
                win_fives.append(act)
            elif opp_val >= self.SCORE_FIVE:
                def_fives.append(act)
            elif my >= self.SCORE_LIVE_FOUR:
                win_fours.append(act)
            elif opp_val >= self.SCORE_LIVE_FOUR:
                def_fours.append(act)
            else:
                scored.append((max(my, int(opp_val * 1.1)), act))
        random.shuffle(win_fives)
        random.shuffle(def_fives)
        random.shuffle(win_fours)
        random.shuffle(def_fours)
        if win_fives:
            result = list(win_fives)
        elif def_fives:
            result = list(def_fives)
        elif def_fours:
            result = list(def_fours)
        elif win_fours:
            result = list(win_fours)
        else:
        ...
        if trans_best is not None and trans_best in result:
            result.remove(trans_best)
            result.insert(0, trans_best)
        elif trans_best is not None:
            result.insert(0, trans_best)
            if len(result) > self.max_candidates + 5:
                result.pop()
        return result
    ...
    def _get_fours_and_defenses_v2(self, state, player):
        """查找所有冲四着法及其防守点（用于 VCT 搜索）. 含防卡死机制。"""
        potential_fours = []
        plate = state.plate
        for r, c in state.candidates:
            self.node_count += 1
    ...
    def _extract_line(self, plate, r, c, dr, dc, override=None):
        """提取经过指定点的行（用于走子前后增量评估）。"""
        line = []
        nr, nc = r - dr, c - dc
        while 0 <= nr < 15 and 0 <= nc < 15:
            line.append(plate[nr * 15 + nc])
            nr -= dr
            nc -= dc
        line.reverse()
    ...
    def apply_move_full(self, state, act):
        """
        在 PrivateState 上执行一步落子（带完整增量更新）。
        
        增量更新: Zobrist哈希、全局评分、候选着法集合。
        
        Returns:
            (old_hash, delta_black, delta_white, added_candidates) 用于悔子
        """
        r, c = act
        player = state.player
        idx = r * 15 + c
        ...
        state.zhash ^= ADAgent.ZOBRIST_TABLE[r][c][player]
        delta_b, delta_w = 0, 0
        ...
        for dr in range(-2, 3):
            for dc in range(-2, 3):
                ...
                if 0 <= nr < 15 and 0 <= nc < 15 and state.plate[nr * 15 + nc] == 0:
                    if (nr, nc) not in state.candidates:
                        added_candidates.add((nr, nc))
    ...
    # ═══════════════════════════════════════════════════
    #  博弈树搜索 (Alpha-Beta + 置换表 + 静态搜索)
    # ═══════════════════════════════════════════════════

    def _evaluate_state(self, state):
        """从当前玩家视角评估状态价值。"""
        if state.player == 2:
            return state.score_white - state.score_black
        else:
    ...
    def _get_opening_move(self, state):
        ...
        if state.player == 1:
            ...
            if len(hist) == 1:
                ...
                return w2
            if len(hist) == 3:
                if self._chosen_opening is None:
                    return None
    ...
    def _reaction(self, state):
        """对弈决策主流程：开局定式 → 防五连 → VCT → Alpha-Beta 搜索。"""
