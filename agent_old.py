# agent.py 融合最终版 (增量缓存优化)
# 整合了两版的长处：丰富的开局库、正确的防守优先级、多杀招随机、根节点近优软选择、
# 同分洗牌去确定性、动态候选窗口等。
# 新增：跨步增量更新，避免每次 get_move 全量重建 PrivateState
...
class Agent(BaseAgent):
    ...
    def get_move(self, state: GameState) -> Tuple[int, int]:
        ...
        # ✅ 搜索完成后缓存当前状态（_reaction 内部用 apply/undo 搜索，结束后状态已恢复）
    ...
    def _get_or_build_private_state(self, state: GameState) -> PrivateState:
        """
        增量构建 PrivateState。
        如果当前状态是缓存状态的后继（多了1~N步），则增量 apply；
        否则全量重建。
        
        支持 N 步增量：collector 中同一 Agent 两次调用间经过对手的 1 步，实际差 2 步。
        """
    ...
    def _generate_candidates(self, plate: bytearray, history: List[Tuple[int, int]]) -> Set[Tuple[int, int]]:
        if not history:
            return {(7, 7)}
        cand = set()
        for r in range(15):
    ...
    def _compute_global_score(self, plate: bytearray) -> Tuple[int, int]:
        score_b, score_w = 0, 0
        for r in range(15):
    ...
    def _evaluate_line(self, line: List[int]) -> Tuple[int, int]:
        score_b, score_w = 0, 0
        n = len(line)
        if n < 5:
            return 0, 0
        for i in range(n - 4):
            ...
            if b_count > 0 and w_count == 0:
                score_b += self._get_pattern_score(b_count, line, i, n)
            elif w_count > 0 and b_count == 0:
                score_w += self._get_pattern_score(w_count, line, i, n)
        return score_b, score_w

    def _get_pattern_score(self, count: int, line: List[int], i: int, n: int) -> int:
        if count == 5:
            return self.SCORE_FIVE
        left_open = (i > 0 and line[i - 1] == 0)
        right_open = (i + 5 < n and line[i + 5] == 0)
        if count == 4:
            if left_open and right_open: return self.SCORE_LIVE_FOUR
            elif left_open or right_open: return self.SCORE_RUSH_FOUR
            else: return 0
        if count == 3:
            return self.SCORE_LIVE_THREE if (left_open and right_open) else (
                self.SCORE_SLEEP_THREE if (left_open or right_open) else 0)
        elif count == 2:
            return self.SCORE_LIVE_TWO if (left_open and right_open) else (
                self.SCORE_SLEEP_TWO if (left_open or right_open) else 0)
        elif count == 1:
            return self.SCORE_LIVE_ONE if (left_open and right_open) else (
                self.SCORE_SLEEP_ONE if (left_open or right_open) else 0)
        return 0

    @staticmethod
    def _is_terminal(state: PrivateState, last_move: Tuple[int, int]) -> Optional[int]:
        r, c = last_move
        plate = state.plate
        player = plate[r * 15 + c]
        if player == 0:
        ...
        for dr, dc in [(1, 0), (0, 1), (1, 1), (1, -1)]:
            ...
            for i in range(1, 5):
                nr, nc = r + dr * i, c + dc * i
                if 0 <= nr < 15 and 0 <= nc < 15 and plate[nr * 15 + nc] == player:
                    count += 1
                else:
                    break
            for i in range(1, 5):
            ...
            if count >= 5:
                return player
        if len(state.history) == 225:
            return 0
        return None

    def _quick_evaluate_point(self, state: PrivateState, r: int, c: int, player: int) -> int:
        best_score = 0
        plate = state.plate
        for dr, dc in [(1, 0), (0, 1), (1, 1), (1, -1)]:
            count, block = 1, 0
            ...
            score = 0
            if count == 4:
                score = self.SCORE_LIVE_FOUR if block == 0 else (self.SCORE_RUSH_FOUR if block == 1 else 0)
            elif count == 3:
                score = self.SCORE_LIVE_THREE if block == 0 else (self.SCORE_SLEEP_THREE if block == 1 else 0)
            elif count == 2:
                score = self.SCORE_LIVE_TWO if block == 0 else (self.SCORE_SLEEP_TWO if block == 1 else 0)
            elif count == 1:
                score = self.SCORE_LIVE_ONE if block == 0 else (self.SCORE_SLEEP_ONE if block == 1 else 0)
            if score > best_score:
                best_score = score
        return best_score

    def _sort_candidates(self, state: PrivateState, trans_best: Optional[Tuple[int, int]] = None) -> List[Tuple[int, int]]:
        ...
        if win_fives: result = list(win_fives)
            ...
            if len(state.history) < 12:
                dynamic_max = min(self.max_candidates + 5, 20)
    ...
    def _gen_threat_moves(self, state: PrivateState) -> List[Tuple[int, int]]:
        moves = []
        player, opp = state.player, 3 - state.player
        for r, c in state.candidates:
            ...
            if score >= self.SCORE_RUSH_FOUR:
                moves.append((score, r, c))
        random.shuffle(moves)
        moves.sort(reverse=True, key=lambda x: x[0])
        return [(r, c) for _, r, c in moves[:8]]

    def _find_five_moves(self, state: PrivateState, player: int) -> List[Tuple[int, int]]:
        fives = []
        plate = state.plate
        for r, c in state.candidates:
            idx = r * 15 + c
            ...
            if self._quick_evaluate_point(state, r, c, player) >= self.SCORE_FIVE:
                plate[idx] = player
                if self._is_terminal(state, (r, c)) == player:
                    fives.append((r, c))
                plate[idx] = 0
        random.shuffle(fives)
        return fives

    def _get_fours_and_defenses_v2(self, state: PrivateState, player: int):
        potential_fours = []
        plate = state.plate
        for r, c in state.candidates:
        ...
        if len(potential_fours) > 15:
            potential_fours = potential_fours[:15]
        fours = []
        for r, c in potential_fours:
            ...
            for r2, c2 in state.candidates:
                ...
                if self._quick_evaluate_point(state, r2, c2, player) >= self.SCORE_FIVE:
                    plate[idx2] = player
                    if self._is_terminal(state, (r2, c2)) == player:
                        five_moves.append((r2, c2))
                    plate[idx2] = 0
            plate[idx] = 0
            if five_moves:
                fours.append(((r, c), list(set(five_moves))))
        return fours

    def _vct_search(self, state: PrivateState, depth: int) -> Optional[Tuple[int, int]]:
    ...
    def _extract_line(self, plate: bytearray, r: int, c: int, dr: int, dc: int,
                      override: Optional[int] = None) -> List[int]:
        line = []
        nr, nc = r - dr, c - dc
    ...
    def apply_move_full(self, state: PrivateState, act: Tuple[int, int]) -> Tuple[int, int, int, Set[Tuple[int, int]]]:
        r, c = act
        player = state.player
        idx = r * 15 + c
        ...
        state.score_black += delta_b
        state.score_white += delta_w
    ...
    def undo_move_full(self, state: PrivateState, act: Tuple[int, int], old_hash: int,
                       delta_b: int, delta_w: int, added_cand: Set[Tuple[int, int]]):
        r, c = act
        ...
        for nr, nc in added_cand:
            ...
            if not has_neighbor:
                state.candidates.discard((nr, nc))

    def _evaluate_state(self, state: PrivateState) -> int:
        if state.player == 2: return state.score_white - state.score_black
        else: return state.score_black - state.score_white

    def _quiescence(self, state: PrivateState, alpha: int, beta: int, depth: int) -> int:
        self.node_count += 1
    ...
    def _lookup_trans(self, state: PrivateState, depth: int):
        key = state.zhash
    ...
    def _store_trans(self, state: PrivateState, depth: int, flag: int, value: int,
    ...
    def _get_opening_move(self, state: PrivateState) -> Optional[Tuple[int, int]]:
        hist = state.history
    ...
    def _reaction(self, state: PrivateState) -> Tuple[int, int]:
    ...
    def _negamax(self, state: PrivateState, depth: int, alpha: int, beta: int):
        self.node_count += 1
        if self.node_count > self.max_nodes:
            self.timeout = True
