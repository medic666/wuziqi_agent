...
提供 Tkinter 图形界面，支持任意两种 Agent 的自动对弈展示。
功能：自动对弈、暂停/继续、手动新局、用时统计、历史记录。

搬自: arena.py (修复: 移除自导入, 兼容新 gamerules, 新增 AZAgent 对战)
"""
...
class Arena:
    """
    可视化对弈竞技场 (Tkinter GUI)。
    
    支持任意两种 Agent 实例的自动对弈，轮流执黑/白。
    显示实时棋盘、用时统计、胜率记录。
    ...
    使用示例:
        ...
        agent1 = ADAgent(depth=4, name="ADAgent")
        agent2 = AZAgent(model_path="model.pt", num_sims=400, name="AlphaZero")
    ...
    def __init__(self, agent_black, agent_white):
        ...
        Args:
            agent_black: 黑方智能体（实现 get_move(state) 方法）
            agent_white: 白方智能体（实现 get_move(state) 方法）
        """
    ...
    def _build_gui(self):
        ...
        ttk.Label(info_lf, textvariable=self.status_var, font=('Arial', 11, 'bold'), 
                  wraplength=250).pack(anchor='w', padx=5, pady=2)
        ...
        ttk.Label(time_lf, textvariable=self.black_time_var, font=('Arial', 10), 
                  foreground='blue').pack(anchor='w', padx=5, pady=2)
        ttk.Label(time_lf, textvariable=self.white_time_var, font=('Arial', 10), 
        ...
        self.stats_text = tk.Text(hist_lf, width=35, height=15, state='disabled', 
    ...
    def start_new_game(self):
        ...
        self.score_var.set(f"总比分 {self.agent_black.name} {self.wins_black} : "
    ...
    def _update_current_game_time(self):
        ...
        self.black_time_var.set(f"● {self.agent_black.name}: 本局步数 {self.cur_agent_black_steps}, "
                               f"平均 {avg_black:.3f}s")
        self.white_time_var.set(f"○ {self.agent_white.name}: 本局步数 {self.cur_agent_white_steps}, "
                               f"平均 {avg_white:.3f}s")
