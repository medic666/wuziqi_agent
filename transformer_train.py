...
"""
五子棋 GoBangTransformer_v2 AlphaZero 训练主循环

基于 az_train.py 改造，使用 GoBangTransformer_v2 替代 ActorCriticNet(CNN)。
训练流程与自对弈、竞技场、基准评估逻辑完全复用现有基础设施。

改动要点:
  - 模型: GoBangTransformer_v2 (Transformer + 2D-RoPE)
  - 配置: TransformerConfig
  - 训练步骤: 适配 Transformer 的优化器参数分组
  - Checkpoint: 保存 model_config 以支持架构自动推断

用法:
    python transformer_train.py                      # 从头训练
    python transformer_train.py --resume             # 续训
    python transformer_train.py --initial_model xxx.pt  # 从预训练模型开始
"""
...
from training.config import TransformerConfig
...
# =====================================================================
# Worker 进程（与 az_train.py 完全相同，仅通过 config 传入参数）
# =====================================================================
...
class TransformerTrainer:
    """GoBangTransformer_v2 AlphaZero 训练器。"""
    ...
    def __init__(self, config: TransformerConfig):
        ...
        self.best_model = GoBangTransformer_v2(**{k: v for k, v in model_config.items() if k != 'arch_type'}).to(self.device)
        self.new_model = GoBangTransformer_v2(**{k: v for k, v in model_config.items() if k != 'arch_type'}).to(self.device)
        ...
        if config.resume:
            ...
            if not loaded:
                logger.error("续训失败：未找到有效检查点！请确认检查点文件存在，或去掉 --resume 参数从头/预训练开始")
                sys.exit(1)
        ...
        if not loaded and config.initial_model:
            if os.path.exists(config.initial_model):
                ...
                logger.warning(f"预训练模型 {config.initial_model} 不存在，跳过")
    ...
    def _create_optimizer(self):
        """创建 AdamW 优化器。Transformer 没有 BN，用 LayerNorm 分组。"""
        decay_params = [p for n, p in self.new_model.named_parameters() if not ('ln' in n or 'bias' in n or 'norm' in n)]
        no_decay_params = [p for n, p in self.new_model.named_parameters() if 'ln' in n or 'bias' in n or 'norm' in n]
    ...
    def _print_header(self):
        ...
        logger.info("=" * 70)
        logger.info("  五子棋 GoBangTransformer_v2 AlphaZero 训练系统")
        logger.info("=" * 70)
        logger.info(f"  网络: Transformer d={c.d_model} H={c.num_heads} L={c.num_layers} | 参数量: {total_p:,}")
        logger.info(f"  Dropout: {c.dropout} | FFN扩展: {c.ff_expand}x")
    ...
    def _train_phase(self, iteration: int):
        ...
        if valid_steps == 0:
            return {
                'train_loss': float('nan'), 'train_policy_loss': float('nan'),
                'train_value_loss': float('nan'), 'lr': new_lr,
                'train_steps': steps, 'valid_steps': 0
            }
    ...
    def _arena_phase(self, iteration: int):
    ...
    def run(self):
        ...
        logger.info("\n✓ GoBangTransformer_v2 AlphaZero 训练完成！")


def main():
    parser = argparse.ArgumentParser(description="GoBangTransformer_v2 AlphaZero 训练")
    parser.add_argument('--initial_model', type=str, default=None, help="预训练模型路径")
